...
const (
	POST_INDEX  = "post"
	DISTANCE    = "200km"
	BUCKET_NAME = "laioffer-lan-bucket"
	ES_URL      = "http://10.128.0.2:9200"
)

var ( // it is actually a hashMap
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	//get token from requestUser
	user := r.Context().Value("user")
	claims := user.(*jwt.Token).Claims
	username := claims.(jwt.MapClaims)["username"]

	//get parameter from request because there is form-data
	// so we call different mathod than handlerSearch
	...
	//Save posts to elasticsearch/ gcs
	...
	//determine the type based on its extension
	...
	id := uuid.New() //create a unique string
	...
	//attrs.MediaLink is where the staff has been placed
	p.Url = attrs.MediaLink
	if p.Type == "image" {
		//uri is the uri of this image
...
func saveToGCS(r io.Reader, objectName string) (*storage.ObjectAttrs, error) {
	...
	//determine whether this bucket is exist through its attri
	if _, err := bucket.Attrs(ctx); err != nil {
		return nil, err
	}

	//wc is the place where to store
	// r is the object we want to store
	...
	//the uploaaded file to free to anyone to read
